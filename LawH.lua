...
local Window = Fluent:CreateWindow({
    Title = "LawHUB",
    SubTitle = "Made With ♥️ By Countra",
    TabWidth = 160,
...
do
    ...
    function TakeEarlyPoint()
        ...
        local Control = game.Players.LocalPlayer.PlayerScripts.PlayerModule.CameraModule.ZoomController.Popper
    if Control and typeof(Control) == "function" then
        for number, value in next, getconstants(Control) do
            if tonumber(value) == 0.25 then
                setconstant(Control, number, 0)
            elseif tonumber(value) == 0 then
                setconstant(Control, number, 0.25) 
            end

        end
  end
        local Check = game.Workspace.Etc.Waypoint.Waypoint.BillboardGui.TextLabel.Text
        game:GetService("Players").LocalPlayer.CameraMaxZoomDistance = 25
        game:GetService("Players").LocalPlayer.CameraMinZoomDistance = 25
        wait(2)
        local starterPrompt = game:GetService("Workspace").Etc.Job.Truck.Starter.Prompt
        local playerName = game.Players.LocalPlayer.Name
        local Plr = game:GetService("Workspace").Lives[playerName].Head
        game:GetService("Workspace").Etc.Job.Truck.Starter.Prompt.Parent = Plr
        wait()
        local Waypoint = game:GetService("Workspace").Etc.Waypoint.Waypoint.Position
...
function TimeTrialJakarta()
    ...
    local checkpointCFrames = {
        ...
        CFrame.new(-21799.7461, 1042.25476, -26797.4766, -0.8445099, 5.20283763e-08, 0.535539925, 2.02707024e-08, 1, -6.51857448e-08, -0.535539925, -4.41942341e-08, -0.8445099),
    }
